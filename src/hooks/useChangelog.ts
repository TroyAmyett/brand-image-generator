...
// Changelog Hook
// Manages changelog entries, unread counts, and read status for Canvas
...
export function useChangelog(userId: string | undefined, product: ChangelogProduct = 'canvas') {
  ...
  const fetchEntries = useCallback(async () => {
    ...
    try {
      const { data, error: fetchError } = await agentpmClient.rpc('get_changelog_entries', {
        p_user_id: userId,
        p_product: product,
        p_limit: 50,
        p_offset: 0,
  ...
  const fetchUnreadCount = useCallback(async () => {
    ...
    try {
      const { data, error: fetchError } = await agentpmClient.rpc('get_unread_changelog_count', {
        p_user_id: userId,
        p_product: product,
      })
  ...
  const fetchUnreadHighlights = useCallback(async () => {
    ...
    try {
      const { data, error: fetchError } = await agentpmClient.rpc('get_unread_highlights', {
        p_user_id: userId,
        p_product: product,
      })
  ...
  const markAsRead = useCallback(async (entryIds: string[]) => {
    if (!agentpmClient || !userId || entryIds.length === 0) return

    try {
      const { error: markError } = await agentpmClient.rpc('mark_changelog_read', {
        p_user_id: userId,
